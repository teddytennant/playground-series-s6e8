...
def main() -> int:
    ...
    if not selected:
        ...
        print("REPRICED TWICE on 2026-08-16, most recently by w16u. w15i's +9.2/+36.5/+112e-6")
        print("ladder is STALE: it was dominated by blend158_logit (CV 0.969961) sitting in a")
        print("top-two tie, and slot 7's 0.97108 pushed it out of BOTH tiers. w16s then priced")
        print("the click at -0.24e-6 to +2.14e-6 at limit 2 — a RANGE, because auto-slot 1 was")
        print("a 1-way tie and the tier-2 tiebreak is undocumented. Slot 9's w16t_cellens4 also")
        print("scored 0.97108, so auto-slot 1 is now a 2-WAY tie and at limit 2 the auto-pick is")
        print("DETERMINED: exactly w16q_ens4avg + w16t_cellens4. Both are ens4-side and both are")
        print("built on the same c_avg, so they can fail together — which is the pairing WANTED")
        print("exists to avoid. Repriced on the same 500 paired draws (w16u_autoprice.json):")
        print("  limit 2: cost of NOT clicking +3.326e-6 +/-0.142, P(auto better) 0.144")
        print("  limit 1: +4.07e-6 (w16q_ens4avg) or +4.16e-6 (w16t_cellens4)")
        print("~2.5 places per 1e-5 at the local density.")
        print("  Still worth clicking, and slot 9 made it MORE worth clicking. Note WHY:")
