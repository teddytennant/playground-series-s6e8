"""w74a — RE-PRICE the final-selection click on the LIVE 131-file board, and on the tau
bound refit today. Successor to w62a_autopair.py; w62a is NOT edited.

Pre-registration: experiments/w74_prereg.txt, committed c2a3f00 BEFORE this file existed.

WHY THIS FILE EXISTS.
w62a priced the determined auto-pair at +4.5228e-6 on a board of 121 scored submissions.
TWENTY more have landed since (ten on 08-23, ten on 08-24) and nobody re-priced it -- which
is the precise failure w57 documented ("nothing runs the gate on a send day") recurring for
the third time. Two inputs move on a send day and BOTH feed the price:

  1. TIER MEMBERSHIP. Checked live: tier 1 is still exactly {w36_ad199stdcorr_ens4,
     w38_ad202stdcorr_ens4} at 0.97119 and tier 2 still the same five at 0.97118. So unlike
     w57a, w62a is NOT void -- its mechanism survives. Only its numbers can drift.
  2. THE beta/gamma/gap GLS FIT, re-fit over whatever files are on the board.

AND A THIRD, WHICH IS THE REASON THIS RUN IS NOT A RESTAMP. RESEARCH:7664 carries a standing
instruction to re-run w19a_transfer.py after every send because "it tightens the tau bound
for free" (194 -> 205 pairs moved the 95% upper 1.72 -> 1.50e-6). Re-run today on 1259 pairs
it went the OTHER WAY: tau_hat still 0.000e-6, but the 95% profile upper is now 2.100e-6.
The bound LOOSENED as evidence accumulated. tau is the one parameter that can turn the click
negative (w62a's P7 sign-flip fires at tau = 5.0), so the sweep here must be read at 2.100
rather than at 1.72, and RESEARCH's "for free" claim is withdrawn.

TWO VERBATIM CONTROLS, because a re-price whose estimator drifted is worthless:
  GATE V  w62a's own -- the estimator reproduces w57a_tierprice2.json on w57a's board.
  GATE W  NEW -- the estimator reproduces w62a_autopair.json (cost, the full counterfactual
          ladder, every solo cost and every tau-sweep row) on w62a's RECORDED 121-file
          board. GATE V alone would pass for anything descended from w57a; GATE W pins this
          file to w62a specifically, which is what a re-price has to be pinned to.

    .venv/bin/python experiments/w74a_clickprice.py

--------------------------------------------------------------------------------------
w62a's original docstring follows unedited, because this file is its body:
...
W62A_COST = 4.522768563707359         # the headline this re-prices (w62a, 121-file board)
TAU_UPPER = 2.100                     # w19a_transfer.py refit 2026-08-24, 1259 pairs (was 1.50)
REPORT_BAR = 1.0                      # w74_prereg P3: republish above this, restamp below
FAILURES = 0
...
def main() -> None:
    ...
    if FAILURES:
        ...
        json.dump(dict(gate_v=False, failures=FAILURES),
                  open(os.path.join(HERE, "w74a_clickprice.json"), "w"), indent=1)
        sys.exit(1)
    print("  ✅ GATE V PASSED — every recorded price reproduces. P2 CONFIRMED.")

    # ------------------------------------------------------------------ GATE W  (w74, NEW)
    # GATE V proves the estimator is w57a's. It does NOT prove it is w62a's: w62a added the
    # determined-pair mechanism, the counterfactual ladder and the tau sweep on top, and a
    # re-price of w62a's headline must reproduce w62a's headline on w62a's OWN board before
    # it is allowed to move it. Same shape as GATE V, one level up the chain.
    print("\n" + "=" * 92)
    print("GATE W — the estimator must reproduce w62a_autopair.json on w62a's OWN 121-file board")
    print("=" * 92)
    ref62 = json.load(open(os.path.join(HERE, "w62a_autopair.json")))
    qNAMES = sorted(ref62["cv"])
    qLB = {k: float(v) for k, v in ref62["lb"].items()}
    qV, qcv = _load(qNAMES, y)
    for k in qNAMES:                       # CV re-derived from the OOF, never re-quoted
        if abs(qcv[k] - float(ref62["cv"][k])) > 5e-10:
            fail(f"GATE W: CV of {k} does not reproduce from the OOF vector")
    qE = build(qNAMES, qLB, qcv, qV, y, beta)
    qmu, qcov, qrsd, qgap = qE["solve"](0.0, CORR_CONS)
    worst_w = abs(qgap - float(ref62["gap"]))
    qAUTO = list(ref62["auto_pair"])
    checks_w = {"cost_auto_pair": qE["price"](qmu, qcov, qrsd, qAUTO)}
    for k, v in ref62["solo"].items():
        checks_w["solo/" + k] = qE["price"](qmu, qcov, qrsd, [k])
    qtie = [k for k in ref62["tier2"] if k in qNAMES]
    A0, B0 = qAUTO
    checks_w["ladder/neither_cleared"] = float(np.mean(
        [qE["price"](qmu, qcov, qrsd, list(p)) for p in itertools.combinations(qtie + [A0, B0], 2)]))
    checks_w["ladder/only_" + A0] = float(np.mean(
        [qE["price"](qmu, qcov, qrsd, [A0, z]) for z in qtie + [B0]]))
    checks_w["ladder/only_" + B0] = float(np.mean(
        [qE["price"](qmu, qcov, qrsd, [B0, z]) for z in qtie + [A0]]))
    checks_w["ladder/click_pick_plus_" + A0] = qE["price"](qmu, qcov, qrsd, [PICK, A0])
    checks_w["ladder/click_pick_plus_" + B0] = qE["price"](qmu, qcov, qrsd, [PICK, B0])
    for t_s, v in ref62["tau_sweep"].items():
        mt, ct, rt, _ = qE["solve"](float(t_s), CORR_CONS)
        checks_w[f"tau/{t_s}"] = qE["price"](mt, ct, rt, qAUTO)
    recorded = {"cost_auto_pair": float(ref62["cost_auto_pair"])}
    recorded.update({"solo/" + k: float(v["cost"]) for k, v in ref62["solo"].items()})
    recorded.update({"ladder/" + k: float(v) for k, v in ref62["ladder"].items()
                     if k != "both_cleared_ACTUAL"})
    recorded.update({f"tau/{k}": float(v) for k, v in ref62["tau_sweep"].items()})
    for k, got in checks_w.items():
        if k not in recorded:
            fail(f"GATE W: nothing recorded for {k}")
            continue
        worst_w = max(worst_w, abs(got - recorded[k]))
        if abs(got - recorded[k]) > 1e-9:
            fail(f"GATE W: {k} {got:+.12f} != recorded {recorded[k]:+.12f}")
    print(f"  {len(qNAMES)} files, {len(checks_w)} recorded quantities re-derived "
          f"(cost + {len(ref62['solo'])} solo + 5 ladder + {len(ref62['tau_sweep'])} tau)")
    print(f"  worst absolute deviation from w62a's artefact: {worst_w:.3e}   (bar 1e-9)")
    if FAILURES:
        print("\n⛔ GATE W FAILED — this is not w62a's estimator. REFUSING to re-price its headline.")
        json.dump(dict(gate_v=True, gate_w=False, failures=FAILURES),
                  open(os.path.join(HERE, "w74a_clickprice.json"), "w"), indent=1)
        sys.exit(1)
    print("  ✅ GATE W PASSED — w62a's headline reproduces exactly. The re-price is like-for-like.")
    ...
    sweep = {}
    for t in [0.0, 0.5, 1.0, 1.72, TAU_UPPER, 2.5, 3.0, 5.0]:
    ...
    print("\n" + "=" * 92)
    print("THE REGISTERED PREDICTIONS (w74_prereg.txt, committed c2a3f00 before this file)")
    print("=" * 92)
    cost_tau_upper = sweep[TAU_UPPER]
    drift = cost - W62A_COST
    p_pick_v = 1.0 if PICK in AUTO else 0.0
    w1 = bool(worst < 1e-9)
    w2 = bool(DETERMINED and set(AUTO) == set(ref62["auto_pair"]))
    w3 = bool(abs(drift) < REPORT_BAR)
    w4 = bool(cost > 0.0 and cost_tau_upper > 0.0)
    w5 = bool(TAU_UPPER <= 1.50)          # registered as PREDICTED-TO-HOLD; already known false
    checks = [
        ("P1 GATE V + GATE W reproduce to < 1e-9", bool(w1 and worst_w < 1e-9),
         f"V {worst:.2e}, W {worst_w:.2e}"),
        ("P2 MECHANISM live tier 1 is still the same determined pair", w2,
         f"tier1={AUTO} n={len(TIER1)}"),
        (f"P3 REPORTING |live - w62a| < {REPORT_BAR:.1f}e-6 (else republish)", w3,
         f"{cost:+.4f} vs {W62A_COST:+.4f}, drift {drift:+.4f}e-6"),
        ("P4 DECISION cost > 0 at tau=0 AND at the fresh 95% upper", w4,
         f"tau0 {cost:+.4f}, tau{TAU_UPPER:.3f} {cost_tau_upper:+.4f}e-6"),
        ("P5 the fresh 95% profile upper on tau is <= 1.50e-6", w5,
         f"upper {TAU_UPPER:.3f}e-6 on 1259 pairs -- the bound LOOSENED"),
    ]
    p1, p3, p4, p5, p6, p7 = w1, w3, w4, w5, bool(p_pick_v == 0.0), bool(not flip)
    p_pick = p_pick_v
    ...
    json.dump(dict(
        gate_v=True, gate_v_worst=worst, gate_w=True, gate_w_worst=worst_w,
        determined=True, w62a_cost=W62A_COST, drift_vs_w62a=drift,
        tau_upper=TAU_UPPER, cost_at_tau_upper=cost_tau_upper,
        ...
        supersedes="w62a_autopair (121-file board, 2026-08-23); mechanism intact, numbers refreshed",
        p1=p1, p2=True, p3=p3, p4=p4, p5=p5, p6=p6, p7=p7, failures=FAILURES,
    ), open(os.path.join(HERE, "w74a_clickprice.json"), "w"), indent=1, default=float)
    print("wrote experiments/w74a_clickprice.json")
